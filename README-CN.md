# 第十一号森林

[English](README.md) · [简体中文](README-CN.md)

一款基于 Phaser 4 的俯视角二次元像素风射击游戏。你将踏入不该存在的第十一号森林 —— 在这里,每片玫瑰花瓣都可能是一颗子弹,每段寂静之下都藏着一首未唱完的歌。

![screenshot](screenshot.png)

## 关于

**第十一号森林** 是一款哥特庭院风的俯视角像素射击游戏,主打手绘质感的二次元美术。玩家扮演一名被森林召来的孤独猎人,在一座座被诅咒的庭园中穿行 —— 玫瑰会还击,大树记得你讲过的每一个名字。

- **类型:** 俯视角动作射击 / Roguelite
- **美术:** 哥特庭院二次元像素风
- **视角:** 俯视 / 动态摄像机跟随与锁定
- **状态:** Alpha / 核心玩法系统与关卡编辑器已完成

## 核心特性

- **数据驱动全配置:** 关卡、角色、怪物 AI、武器弹道、掉落物、音效规则全量基于 YAML + Schema 强校验驱动。
- **动态选角与酒馆流转:** 支持多角色（包含独立配音与伤害路由）、酒馆场景交互选角与存档持久化。
- **流畅战斗与 AI 动作机制:** 包含 Matter.js 物理刚体碰撞、基于脚底 BodyBox 的怪物寻路算法、击退/僵直/受击状态机与自定义武器 Z 轴深度层级。
- **无缝关卡与传送门系统:** 场景间通过配置化的传送门 (Teleporter) 实现顺滑切换与数据传递。
- **可视化在线编辑器:** 内置强大独立的关卡编辑器，支持空气墙绘制、怪物放置、传送门设置、地图模块拼贴及 Sprite 帧剪裁预览，编辑结果直接通过后端 API 持久化写回 YAML 数据文件。
- **AI 智能生成管线:** 深度集成 MiniMax AI 音乐/音效生成与 Google Gemini 3 Pro Image (Nano-banana 2) 像素美术生成。

## 技术栈

| 层       | 选择                                                                              |
| -------- | --------------------------------------------------------------------------------- |
| 游戏引擎 | [Phaser 4](https://github.com/phaserjs/phaser)                                    |
| UI       | [React 19](https://react.dev)                                                     |
| 构建     | [Vite 6](https://vite.dev)                                                        |
| 语言     | [TypeScript 5.7](https://www.typescriptlang.org)                                  |
| 音乐     | [MiniMax](https://MiniMax.io) AI 音乐生成                                         |
| 美术生成 | [Google Nano-banana 2](https://ai.google.dev/gemini-api/docs)(Gemini 3 Pro Image) |

## 快速开始

环境要求: Node.js 18+。包管理器: **pnpm**。

```bash
pnpm install
pnpm dev          # 启动开发服务器(带匿名统计)
pnpm dev-nolog    # 启动开发服务器(不带统计)
pnpm build        # 生产构建,产物输出到 dist/
```

开发服务器默认运行在 `http://localhost:8080`。

## 项目结构

| 路径                      | 说明                                           |
| ------------------------- | ---------------------------------------------- |
| `index.html`              | HTML 外壳 + 启动画面                           |
| `src/main.tsx`            | React 入口                                     |
| `src/App.tsx`             | 顶层 React 组件                                |
| `src/PhaserGame.tsx`      | Phaser 与 React 之间的桥接                     |
| `src/game/`               | 游戏源码,每个模块一个目录                      |
| `src/game/main.ts`        | Phaser 游戏配置与启动                          |
| `src/game/scenes/`        | `LoadScene` —— 所有关卡共用的通用场景          |
| `src/lib/events/bus.ts`   | 跨边界事件总线                                 |
| `src/store/game-store.ts` | HUD 状态 + 持久化存档                          |
| `public/data/`            | 关卡 / 怪物 / 武器 / … 的 YAML                 |
| `public/assets/`          | 静态资源(贴图、音频)                           |
| `docs/`                   | 长文档,见 [`docs/README.md`](./docs/README.md) |

## AI 生成音乐

游戏配乐通过 **MiniMax** AI 音乐生成服务制作。每一首曲目按游戏氛围打标 prompt(例如 _"暮色玫瑰庭院,缓钢琴配低音弦乐"_),生成结果落到 `public/assets/audio/` 下。

如需重新生成某首曲子,使用 MiniMax 音乐 API,传入同目录下 `.prompt.json` 中保存的 prompt 即可。

## AI 生成美术

像素美术等视觉资源通过 Google 的 **"Nano-banana 2"**(又名 [Gemini 3 Pro Image](https://ai.google.dev/gemini-api/docs/image-generation),原生多模态图像模型)生成。每项资产的 prompt 模板存放在 [`prompts/`](./prompts/),生成结果落到 [`public/assets/image/`](./public/assets/image/)。

需要重新生成某个资产时,调用 Nano-banana 2 API(`https://generativelanguage.googleapis.com/v1beta`),传入对应模板的 prompt,把返回结果写入 `public/assets/image/<name>.png`。

API key 在 `.env` 中配置:

```env
VITE_GEMINI_API_KEY=...
VITE_GEMINI_MODEL=gemini-3-pro-image
```

## 开发提示

直接编辑 `src/` 下任何文件,Vite 会自动热更新。

没有"每个关卡一个场景类"这回事 —— `LoadScene` 按 YAML 渲染所有关卡。新增关卡只需写
`public/data/levels/<id>.yaml` 并把 id 加进 `index.yaml`,完整流程见
[`docs/SCENES.md`](./docs/SCENES.md)。

日常维护游戏数据(新增角色、调整怪物数值、重新生成音效),见 [`docs/SKILL.md`](./docs/SKILL.md),里面讲了 schema 优先的工作流、平衡性参考值以及 AI 资产生成管线。

## 文档

详细的设计、机制、美术与工具文档存放在 [`docs/`](./docs/),每个主题一个 `UPPERCASE-ENGLISH.md` 文件。完整索引见 [`docs/README.md`](./docs/README.md)。

建议从这里开始:

- [`docs/SCENES.md`](./docs/SCENES.md) — 场景如何被加载(文件结构、`handle-fetch`、如何添加新场景、怪物刷新触发条件、HUD 桥接)。
- [`docs/MODULES.md`](./docs/MODULES.md) — 每个数据驱动模块都遵循的 `yaml → schema → parser → loader → logic` 模式。
- [`docs/SKILL.md`](./docs/SKILL.md) — 如何维护 `public/data/` 目录(schema、平衡性、AI 资产生成)。

各模块参考:

- [`docs/WEAPONS.md`](./docs/WEAPONS.md) · [`docs/MONSTERS.md`](./docs/MONSTERS.md) · [`docs/CHARACTERS.md`](./docs/CHARACTERS.md) · [`docs/DROPS.md`](./docs/DROPS.md) · [`docs/AUDIOS.md`](./docs/AUDIOS.md) · [`docs/EVENTS.md`](./docs/EVENTS.md) · [`docs/PERSIST.md`](./docs/PERSIST.md) · [`docs/EDITOR.md`](./docs/EDITOR.md)

## 许可证

MIT —— 见 [LICENSE](./LICENSE)。

> 原始 Phaser + React + TypeScript 模板版权 © Phaser Studio Inc.,MIT 协议。
