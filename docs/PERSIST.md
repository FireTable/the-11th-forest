...
Deliberately **not** persisted: `isDead`, `hubsVisible`, `levelTitle`, `isReloading`, `reloadProgress`, `activeAmmo` / `activeMaxAmmo` (derived from `slots`). Transient or recomputed on load — persisting them would only create ways to restore into an inconsistent state.
...
Two ways a save is discarded, both deliberate:
...
- **Teleport to another level** — `TeleporterController` sets the new `currentLevelId` and clears the entity snapshots, so the next level spawns from its own YAML instead of inheriting the previous level's monsters.
