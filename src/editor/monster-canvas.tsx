 ...
 *
 * Read-only visuals — markers can be CLICK-DRAGGED to reposition the
 * spawn in image space; release commits via `onSpawnMove`. Coordinates
 * are also editable in the form list.
 *
 * Active when `sceneSubTab === 'monsters'` so it doesn't draw over the
 * air-wall canvas.
 *
 ...
 * spawn in image space; release commits via `onMove`. Renders one
 * idle frame from the spritesheet — clipped via overflow:hidden on
 * the wrapper + a positioned full-sheet <img> inside.
 */
function DraggableMarker({
    ...
    return (
        ...
        >
            ...
            />
            {/* Sprite thumbnail — drag handle. pointer-events-auto so
                it receives the drag even though the rest of the overlay
                is non-interactive. overflow-hidden + a positioned full
